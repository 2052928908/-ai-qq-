...
点击编辑系统环境变量  
点击编辑  
新建一个环境变量，将python.exe的路径输入进去  
(python.exe默认地址是<code>C:\Users\Administrator\AppData\Local\Programs\Python\Python<版本号>\python.exe</code>)  
点击确定 
## 安装ollama：
...
>无法解压的可以下载[bandzip](https://www.bandisoft.com/bandizip/dl.php?web)，安装评价版再解压  
完成后进入astrbot目录，在本目录下进入powershell:  
<code>推荐方法:</code>在文件夹里上方有路径，直接点击就会出现一串路径，输入powershell  
<code>备用方法:</code>点击win+r键，输入powershell并回车，利用cd命令<code>cd [你解压的路径]</code>  
确定路径后将本路径文件夹中的<code>launcher_astrbot_en.bat</code>拖入powershell里.输入一个空格后输入<code>server</code>并回车
### 进入astrbot
在浏览器输入:
```markdown
localhost:6185
...
#### AI配置
- Agent执行方式 : ai对话的执行器,包括Dify,Coze等执行器,本期以本地为主
- 模型 : 在模型提供商中会启用多种模型,在这里可以设置具体的回复模型,可选择在各种情况下如默认模型调用失败或默认模型为纯文本时被输入图片等情况下用其他何种模型进行替代
- 人格 : ai回复的人格设定,可在人格设定里添加人格(在这里进行人格的塑造)
- 流式输出 : 可以将ai对话实时生成的部分显示出来,可以在astrbot的网页中对话中更加顺畅,目前astrbot并不支持qq的流式回复(napcat实质上是把ai输出伪装成用户输出,而qq不支持预览其他用户输入框中的内容)
- 健康模式 : 避免敏感词汇,让ai输出更有益的内容
#### 平台配置
- 管理员 ID : 可设置管理员id,默认为astrbot
- 隔离会话 : 可专注与发送人的对话,不在参考群中其他人的上下文对话
- 唤醒词 : 可设置独特唤醒词,可唤醒ai进行对话,默认私聊不需要唤醒词,@ai时会自动唤醒
#### 插件配置
...
## 扩展
- 插件&插件市场 : 可以添加各种插件完成更加个性化的需求,添加所有你想要的功能
- MCP : 赋予ai一定的权限,让它能做到一些事情如发一封邮件,运行代码等,让ai不再局限于聊天框,而是有改变现实的能力
- skills : 更专业的执行策略,让ai更专业的执行某种任务,使其更加高效,高质量的完成任务要求
- 管理行为 : 可对ai下达指令以完成特殊的任务
## 更多
- 对话数据 : 可监视使用情况,观察有哪些任务通过什么方式输出对话
- 自定义规则 : 如添加安全词等特殊内容
- 未来任务管理 : 当你认定的时间到达,astrbot会自动唤醒并完成你布置的任务
- SubAgent 编排 : 让ai分工合作,各取所长以达成更好的效果
- 数据统计 : 让数据可视化,更方便观察其运行状态
- 平台日志 : 查看后台都在做什么,更精确的观察哪里出现问题,针对性地解决问题
# 👥联系我
